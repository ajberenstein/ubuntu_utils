...
sudo sed -i 's|^\(rsession-\).*||g' /etc/rstudio/rserver.conf	# remove existing configuration if exist
sudo sed -i '/^$/d' /etc/rstudio/rserver.conf			# remove empty line if were necesary
newline="rsession-which-r=/opt/R/${R_VERSION}/bin/R"		# newline to add into config file
echo $newline | sudo tee -a /etc/rstudio/rserver.conf		## adding new configuration for launching rstudio server with the required version
